...
$s=mysqli_query($connect,"SELECT count(*) as count FROM login WHERE email='$email'");
	...
	if($display['count']>0)
		  {
			  echo "<script>alert('This email is already exist');window.location='member entry form.php'</script>";	
		  }
...
$qry = "INSERT INTO users(fullname,email,password,gender,service,plans,address,phoneno,dob,amount) values ('$fullname','$email','$password','$gender','$service','$plans','$address','$phoneno','$dob','$totalamount')";
